---
name: auto-agent-teams
---

# Agent Teams Skill

> ⚠️ **Platform Note**: Agent Teams is a **Claude Code-exclusive** runtime feature. Gemini CLI does not implement `TeamCreate`, `SendMessage`, or team-scoped spawning APIs. This skill is kept for **cross-platform documentation parity only** — do not attempt to execute its patterns on Gemini CLI.
>
> On Gemini CLI, `--team` flag is ignored with a warning and the pipeline falls back to the standard subagent mode (Route A). Refer to Claude Code's agent-teams skill for the authoritative specification: https://code.claude.com/docs/en/agent-teams
>
> The sections below describe the **Claude Code** feature and are non-authoritative for Gemini CLI operators.
...
- Joins the team as a teammate — the **top-level session** creates the team and spawns Lead; Lead MUST NOT call `TeamCreate` itself (nested team creation is rejected on Claude Code, and unsupported on Gemini CLI).
...
| Guardian teammate fails | Lead falls back to subagent validator with `@validator` |
...
**Ref**: SPEC-WORKTREE-001, `@.gemini/skills/autopus/worktree-isolation.md`
