...
When the default subagent pipeline runs Phase 2 with parallel tasks, each executor agent receives `auto pipeline worktree` so Claude Code places it in a separate git worktree. After all parallel agents complete, their branches are merged sequentially (Phase 2.1) before Gate 2 validation.
...
For parallel tasks in Phase 2, add `auto pipeline worktree` to spawn_agent() calls:

```python
# Parallel execution with worktree isolation
result_t1 = spawn_agent(
    agent_type = "executor",
    message = "Implement T1: ...",
      isolation = "worktree"   # R1: each executor gets an independent worktree
)
result_t2 = spawn_agent(
    agent_type = "executor",
    message = "Implement T2: ...",
      isolation = "worktree"
)
```
...
Sequential tasks do NOT use `auto pipeline worktree`:

```python
# Sequential — main worktree
result_t3 = spawn_agent executor --task "Implement T3 using T1 result: ..."
```
...
Codex should not assume implicit worktree provisioning from agent flags. Use the documented worktree-isolation procedure when branch separation is required.
...
Note: Claude Code auto-generates branch names when `auto pipeline worktree` is used. This convention is advisory for log traceability and manual worktree creation.
