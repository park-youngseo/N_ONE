...
For Agent Teams mode (`--team`), see `.codex/skills/agent-teams.md` for role-based team composition (Lead/Builder/Guardian).

@.codex/skills/worktree-isolation.md
...
The quality mode — determined by the `--quality` flag or interactive selection — controls the execution profile for spawn_agent() calls.

### Ultra Mode

Use the premium path for all spawn_agent() calls.
...
Omit the `model` parameter in spawn_agent() calls to use each agent definition's frontmatter model:
...
In Balanced mode, task complexity determines the profile per spawn_agent() call:
...
Step 2: Fetch Documentation (for each detected library, max 5)
  → Call WebSearch "libraryName docs"
  → If no match: log "[CTX7] No match: {name}", skip to next
  → Call WebSearch "libraryId, topic="{task-relevant topic} docs"
...
Ref: `.codex/rules/autopus/context7-docs.md` for detection heuristics, token limits, and anti-patterns.

### Phase 2: Implementation

Tasks that can run in parallel are spawned with multiple spawn_agent() calls in a single message.

Parallel tasks use `auto pipeline worktree` so each executor works in an independent git worktree (R1). Max 5 concurrent worktrees; overflow tasks are queued.
...
spawn_agent executor --task "Implement T1: ..."  # Balanced
spawn_agent executor  # Ultra
spawn_agent executor --task "Implement T2: ..."  # Balanced
spawn_agent executor  # Ultra
```

Collect `worktree_path` and `branch` from each return value for Phase 2.1 merge.

Sequential tasks do NOT use `auto pipeline worktree` and merge immediately after completion before the next dependent task is spawned (R3).

```
# Sequential execution example — immediate merge after each task
result_t1 = spawn_agent executor --task "Implement T1: ..."
# merge T1 worktree branch immediately (if isolation was used), then spawn T2
spawn_agent executor --task "Implement T2. T1 result: {result_t1}"
```
...
See @.codex/skills/worktree-isolation.md for full merge strategy and safety rules.
...
spawn_agent(
  ...
  message = """
    ...
    Reference: .codex/skills/frontend-verify.md for the full pipeline.
...
spawn_agent(agent_type = "reviewer", message = """
    ...
    Issues: <list of issues>
""")
spawn_agent(agent_type = "security-auditor", message = """
...
|-----------------------------------------------|-------------------|--------------------|
| planner specifies Mode = "parallel"           | Parallel          | Yes (`auto pipeline worktree`) |
...
|---------------------------|----------------------------------------------------|
| Exits due to maxTurns     | Detect remaining work → spawn new spawn_agent()          |
