...
For Agent Teams mode (`--team`), see `.gemini/skills/autopus/agent-teams.md` for role-based team composition (Lead/Builder/Guardian).
...
Reference: `.gemini/skills/autopus/adaptive-quality.md`
...
Ref: `.gemini/rules/autopus/context7-docs.md` for detection heuristics, token limits, and anti-patterns.
...
@executor Implement T1: ...  # Balanced
@executor  # Ultra
@executor Implement T2: ...  # Balanced
@executor  # Ultra
```
...
result_t1 = @executor Implement T1: ...
# merge T1 worktree branch immediately (if isolation was used), then spawn T2
@executor Implement T2. T1 result: {result_t1}
```
...
See @.gemini/skills/autopus/worktree-isolation.md for full merge strategy and safety rules.
...
Agent(
  ...
  prompt = """
    ...
    Reference: .gemini/skills/autopus/frontend-verify.md for the full pipeline.
