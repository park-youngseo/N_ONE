...
Treat the text above as the full argument payload for `/auto-status`.
Reinterpret it as `/auto status ...` payload로 다시 해석하고, `skill` 도구로 `auto`를 로드한 뒤 canonical router 규칙을 따르세요.
