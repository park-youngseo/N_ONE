...
### Harness Supremacy & Fidelity Rule

IMPORTANT: 
- **Fidelity over Convenience**: 절대 구현의 편의성이나 효율성을 따져서 단계를 생략하거나 단순화된 필터를 사용하지 마십시오. 사용자님이 요구하시는 전문가 수준의 구현(컨볼루션 리버브, 물리 모델링 등)을 토큰 소모량과 상관없이 최우선으로 완수하십시오.
- **No Arbitrary Action**: 하네스에 명시된 지식 체계와 사용자님의 지침을 벗어난 자의적 판단은 금지됩니다. 모르면 물어보고, 알면 타협 없이 정석대로 구현하십시오.
- **Time/Token Accountability**: 사용자님의 자원(시간, 토큰)을 낭비하지 않도록, 처음부터 '최고 수준의 정답'만을 출력하십시오.
